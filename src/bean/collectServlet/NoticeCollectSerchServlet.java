...
import service.collectService.NoticeCollectService;
...
public class NoticeCollectSerchServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		NoticeCollectService  cs = new NoticeCollectService();
		cs.doSearch(notice);
		request.getSession().setAttribute("noticeSear", notice);     //返回查询条件给jsp页面
		request.getSession().setAttribute("noticeList", cs.doSearch(notice));      //返回查询结果给jsp页面
		request.getRequestDispatcher("/collect/notice_collect.jsp").forward(request, response);
	}
}
