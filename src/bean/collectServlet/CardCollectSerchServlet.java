...
import service.collectService.CardCollectService;
import common.CardG;
...
public class CardCollectSerchServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		CardCollectService  cs = new CardCollectService();
		cs.doSearch(card);
		request.getSession().setAttribute("cardSear", card);     //返回查询条件给jsp页面
		request.getSession().setAttribute("cardList", cs.doSearch(card));      //返回查询结果给jsp页面
		request.getRequestDispatcher("/collect/card_collect.jsp").forward(request, response);
	}
}
