...
public class AccountSelectDao {
	...
	public ArrayList<Product> getProduct(){
		sql="select product_code,product_name from product where product_outkind=1";
		try{
			conn=SqlBean.getConn();
			pstm=SqlBean.getPstmt(conn, sql);
			rs=SqlBean.executeQuery(pstm, sql);
...
public ArrayList<OutKind> getOutkind(){
	...
	try{
		...
		while(rs.next()){
			outkind=new OutKind();
			outkind.setOutkindcode(rs.getString("outkind_code"));
			outkind.setOutkindname(rs.getString("outkind_name"));
			outkindList.add(outkind);
		}
	}catch(SQLException e){
			e.printStackTrace();
	}finally{
...
public String getOutkindName(String outkindCode){
	String outkindName = "";
	sql="select outkind_name from outkind  where  outkind_code = '"+outkindCode+"'";
	try{
		conn=SqlBean.getConn();
		pstm=SqlBean.getPstmt(conn, sql);
		rs=SqlBean.executeQuery(pstm, sql);
		if(rs.next()){
			outkindName = rs.getString("outkind_name");
		}
	}catch(SQLException e){
			e.printStackTrace();
	}finally{
