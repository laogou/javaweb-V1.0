...
<center>
  <form action="PreCheckSerchServlet" method="post" onSubmit="return checkTime(document.getElementById('from').value,document.getElementById('to').value)">
  <%
...
<center>
  <form action="PreCheckServlet" method="post" >
    ...
    <%
      ...
      int iNum;
      try{
         preList = (ArrayList<PreG>)request.getSession().getAttribute("checkList");
        ...
        <%
             for(;i<=preList.size();i++){
                ...
                //String preSerial = "preId"+i;
                //String preType = "preType"+i;
                //Integer preID = css.get
                
                //Integer serial = pre.getPreserial();
      ...
      %>
        ...
        <td><input name="amount" type="text" size="10" value="<%=amount%>" readonly="readonly" /></td>
      	<td><input name="update" value="通    过" type="button" onclick="needUpdate(<%=i%>,'<%=preType%>')"/></td>
        <td><input name="delete" value="不通过" type="button" onclick="needDelete(<%=i%>,'<%=preType%>')"/></td>
      </tr>
      <%
          }
        }
