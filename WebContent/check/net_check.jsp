...
<center>
  <form action="NetCheckSerchServlet" method="post" onSubmit="return checkTime(document.getElementById('from').value,document.getElementById('to').value)">
  <%
...
<center>
  <form action="NetCheckServlet" method="post" >
    ...
    <%
      ...
      int iNum;
      try{
         netList = (ArrayList<NetG>)request.getSession().getAttribute("checkList");
        ...
        <%
             for(;i<=netList.size();i++){
                ...
                //String netSerial = "netId"+i;
                //String netType = "netType"+i;
                //Integer netID = css.get
                
                //Integer serial = net.getNetserial();
      ...
      %>
        ...
        <td><input name="amount" type="text" size="10" value="<%=amount%>" readonly="readonly" /></td>
      	<td><input name="update" value="通    过" type="button" onclick="needUpdate(<%=i%>,'<%=netType%>')"/></td>
        <td><input name="delete" value="不通过" type="button" onclick="needDelete(<%=i%>,'<%=netType%>')"/></td>
      </tr>
      <%
          }
        }
