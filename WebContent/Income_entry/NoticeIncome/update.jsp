...
<form name="QuerryUserFrom" method="post"> 
	...
	<center>
        ...
        %>
		    <tr>
		        <td>流水号：</td>
		        <td nmae="serial" value="serial"><%=notice.getSerial() %></td>
		    </tr>
		    <tr>
		        <td>营业收款日期：</td><td><input type="date" name="cz_month" value=<%=notice.getDate()%> /></td>
		    </tr>
			<tr>
				<td>城市编码：</td>
				<td>
					<select name="country_name">
					<option value=<%=notice.getCity_code()%>><%=notice.getCity_code()%></option>
					<% 
			...
			<tr>
				...
				<td>
					...
					<option value=<%=notice.getProduct_code()%>><%=notice.getProduct_code()%></option>
					<% 
			...
			<tr>
				...
				<td>
					...
					<option value=<%=notice.getNotice_code()%>><%=notice.getNotice_code()%></option>
						<% 
						...
						try {
				            ...
				            String sql = "select notice_name from notice";
				            pstmt = conn.prepareStatement(sql);
				            rs = pstmt.executeQuery();
				            System.out.println(sql);
							rs=pstmt.executeQuery();
						...
						{
							%>
							<option value="<%=rs.getString("notice_name")%>"><%=rs.getString("notice_name")%></option>
							<%
						}
			...
			<tr>
				<td>营业收入金额：</td><td><input type="text" name="input_money" value=<%=notice.getAmount()%> /></td>
			</tr>
			<tr>
				<td colspan="2" align="center"><input  type="submit" value="修改" 
			onClick= "QuerryUserFrom.action='UpdateNoticeServlet?serial=<%=notice.getSerial() %>';QuerryUserFrom.submit()"></td>
			</tr>
